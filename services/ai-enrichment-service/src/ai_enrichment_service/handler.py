from __future__ import annotations

import os

from ai_enrichment_service.embedder import BedrockEmbeddingGenerator, InMemoryEmbeddingGenerator
from ai_enrichment_service.generator import BedrockEnrichmentGenerator, DeterministicEnrichmentGenerator
from ai_enrichment_service.indexer import InMemoryVectorIndexer, OpenSearchVectorIndexer
from ai_enrichment_service.publisher import InMemoryTopicPublisher, SNSTopicPublisher
from ai_enrichment_service.store import AuroraAIEnrichmentStore, InMemoryMetadataStore
from ai_enrichment_service.service import AIEnrichmentService
from shared.responses import json_response


def lambda_handler(event: dict, _context) -> dict:
    publisher = _build_publisher()
    metadata_store = _build_metadata_store()
    generator = _build_generator()
    embedding_generator = _build_embedding_generator()
    vector_indexer = _build_vector_indexer()
    result = AIEnrichmentService(
        publisher=publisher,
        metadata_store=metadata_store,
        generator=generator,
        embedding_generator=embedding_generator,
        vector_indexer=vector_indexer,
    ).enrich(event)
    return json_response(202, {"message": "Meeting intelligence generated.", "snsEvent": result})


def _build_publisher():
    if os.getenv("MEETING_INTELLIGENCE_TOPIC_ARN"):
        return SNSTopicPublisher()
    return InMemoryTopicPublisher()


def _build_metadata_store():
    if os.getenv("AURORA_DATABASE_URL") or os.getenv("DATABASE_URL"):
        return AuroraAIEnrichmentStore()
    return InMemoryMetadataStore()


def _build_generator():
    if os.getenv("BEDROCK_MODEL_ID"):
        return BedrockEnrichmentGenerator()
    return DeterministicEnrichmentGenerator()


def _build_embedding_generator():
    if os.getenv("BEDROCK_EMBEDDING_MODEL_ID"):
        return BedrockEmbeddingGenerator()
    return InMemoryEmbeddingGenerator()


def _build_vector_indexer():
    if os.getenv("OPENSEARCH_ENDPOINT"):
        return OpenSearchVectorIndexer()
    return InMemoryVectorIndexer()
